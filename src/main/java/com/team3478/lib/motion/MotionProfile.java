...
// successively coincident
//  * MotionSegments from which the desired state of motion at any given distance or time can be
// calculated.
// Authors: -
// Notes:
...
public class MotionProfile {
  ...
  public Optional<MotionState> firstStateByPos(double pos) {
    for (MotionSegment s : mSegments) {
      if (s.containsPos(pos)) {
        ...
        if (Double.isNaN(t)) {
          LoggerManager.log(LogLevel.ERROR, "Error! We should reach 'pos' but we don't");
          return Optional.empty();
        }
        return Optional.of(s.start().extrapolate(t));
      }
  ...
  public void appendControl(double acc, double dt) {
    if (isEmpty()) {
      LoggerManager.log(LogLevel.ERROR, "Error!  Trying to append to empty profile");
      return;
    }
