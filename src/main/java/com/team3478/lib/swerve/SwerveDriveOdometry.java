...
public class SwerveDriveOdometry implements CSVWritable {
  ...
  public Pose2d updateWithWheelConstraints(
      double currentTimeSeconds, Rotation2d gyroAngle, SwerveModuleState... moduleStates) {
    double period = m_prevTimeSeconds >= 0 ? currentTimeSeconds - m_prevTimeSeconds : 0.0;
    // LoggerManager.log(LogLevel.INFO,"Time: " + currentTimeSeconds);
    m_prevTimeSeconds = currentTimeSeconds;
    ...
    // LoggerManager.log(LogLevel.INFO,chassisState);
    ...
    // LoggerManager.log(LogLevel.INFO,"Translation: " + newPose);
    m_velocity = chassisState;
    // m_velocity.omegaRadiansPerSecond = m_previousAngle.inverse().rotateBy(gyroAngle).getRadians()
    // / period;
    m_poseMeters = new Pose2d(m_poseMeters.transformBy(newPose).getTranslation(), angle);
    m_previousAngle = angle;
    // LoggerManager.log(LogLevel.INFO,m_poseMeters);
    return m_poseMeters;
  }
  ...
  public String toString() {
    return m_poseMeters.toString() + " - " + m_velocity.toString();
  }

  @Override
  public String toCSV() {
    return m_poseMeters.toCSV() + "," + m_velocity.toCSV() + "," + m_prevTimeSeconds;
  }

  @Override
  public String toCSVHeader() {
    StringBuilder builder = new StringBuilder();
    builder.append(m_poseMeters.toCSVHeader());
    builder.append(",");
    builder.append(m_velocity.toCSVHeader());
    builder.append(",");
    builder.append("prev_time");
