...
public class QuinticHermitePoseSplineNonholonomic extends PoseSpline {
  ...
  protected QuinticHermitePoseSplineNonholonomic(
      QuinticHermiteSpline1d x, QuinticHermiteSpline1d y) {
    this.x = x;
    this.y = y;
  }

  // Return a new spline that is a copy of this one, but with adjustments to second derivatives.
  protected QuinticHermitePoseSplineNonholonomic adjustSecondDerivatives(
      double ddx0_adjustment,
      double ddx1_adjustment,
      double ddy0_adjustment,
      double ddy1_adjustment) {
  ...
  public Pose2d getStartPose() {
    return new Pose2d(getPoint(0), getHeading(0));
  }

  public Pose2d getEndPose() {
    return new Pose2d(getPoint(1), getHeading(1));
  }
  ...
  private static void runOptimizationIteration(List<QuinticHermitePoseSplineNonholonomic> splines) {
    ...
    for (int i = 0;
      ...
      splines.set(
          ...
          splines
              .get(i)
              .adjustSecondDerivatives(0, -controlPoints[i].ddx, 0, -controlPoints[i].ddy));
      splines.set(
          i + 1,
          splines
              .get(i + 1)
              .adjustSecondDerivatives(-controlPoints[i].ddx, 0, -controlPoints[i].ddy, 0));
    }
    ...
    for (int i = 0;
      ...
      splines.set(
          ...
          splines
              .get(i)
              .adjustSecondDerivatives(0, 2 * controlPoints[i].ddx, 0, 2 * controlPoints[i].ddy));
      splines.set(
          i + 1,
          splines
              .get(i + 1)
              .adjustSecondDerivatives(2 * controlPoints[i].ddx, 0, 2 * controlPoints[i].ddy, 0));
    }
    ...
    for (int i = 0; i < splines.size() - 1; ++i) {
      ...
      splines.set(
          i,
          splines.get(i).adjustSecondDerivatives(0, controlPoints[i].ddx, 0, controlPoints[i].ddy));
      splines.set(
          i + 1,
          splines
              .get(i + 1)
